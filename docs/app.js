...
function renderPortfolio() {
  ...
  if (pageHeader && !pageHeader.querySelector('.btn-refresh')) {
    pageHeader.insertAdjacentHTML('afterbegin', `<button onclick="refreshPrices()" class="btn-refresh" title="Refresh live prices" style="font-size:11px;font-weight:700;padding:7px 14px;border:1.5px solid #00D4A0;border-radius:6px;background:#00D4A0;color:#0A0A0A;cursor:pointer;font-family:inherit">🔄 Refresh</button>`);
  }
...
function _buildTechCard(symbol, tech, intel, uid) {
  ...
  if (tech && overall && overall !== '-') {
    ...
    verdictHtml = `
      <div class="tsc-verdict-section">
        <div class="tsc-verdict-label">ENTRY TIMING</div>
        ...
        <div class="tsc-disclaimer">Use for entry timing only - not a sell signal for long-term holders.</div>
      </div>`;
  } else {
  ...
  if (savedAnalysis && savedAnalysis.analysis_text) {
    const savedTs = savedAnalysis.analyzed_at
      ...
      : '';
    const savedAction = detectActionFromAnalysis(savedAnalysis.analysis_text);
    // DIVIDEND THESIS badge (separate from Entry Timing)
    savedAiVerdictHtml = `
